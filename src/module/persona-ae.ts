...
import { BUFF_MAX_POTENCY, FatigueStatusId } from "../config/status-effects.js";
...
export class PersonaAE extends ActiveEffect<PersonaActor, PersonaItem> implements ModifierContainer<PersonaAE> {
  ...
  get potency(): number {
    try {
      ...
      return Number.isNaN(potency) || potency == 0 ? 1 : potency;
    } catch {
  ...
  async setPotency(potency: number) : Promise<void> {
    potency = Math.clamp(potency, 1 , this.maxPotency);
    let tint = "#ffffff";
    if (potency == 2) {
      if (this.isBuffStatus) {
        tint= "#00ff00";
      }
      if (this.isDebuffStatus) {
        tint = "#ff0000";
      }
    }
    if (tint != this._source.tint) {
      await this.update({tint});
    }
  ...
  async mergeStatus(status: StatusEffect) {
    await this.mergeDuration(status.duration);
    const extraPotency = status.potency || 1;
    const newPotency = Math.min(this.potency + extraPotency, this.maxPotency);
    if (newPotency > this.potency) {
      await this.setPotency(newPotency);
    }
  }

  async reducePotency(amt: number) : Promise<number> {
    const newPotency = this.potency - (amt || 1);
    if (newPotency <= 0) {await this.delete();}
    if (newPotency > 0) {
      await this.setPotency(newPotency);
    }
    return Math.abs(Math.min(0 , newPotency));
  }
  ...
  private async mergeDuration(newDuration : StatusDuration) : Promise<void> {
    ...
    switch (newDuration.dtype) {
      ...
      case "3-rounds":
        if ("amount" in oldDuration) {
          oldDuration.amount= Math.max(oldDuration.amount, newDuration.amount);
  ...
  async onAEDelete() : Promise<void> {
    if (!game.user.isGM && PersonaSettings.debugMode()) {
      const acceptableStatusDelete : StatusEffectId[] = [
        "down",
        "bonus-action",
        "baton-pass",
      ];
      if (!acceptableStatusDelete.some (x=> this.statuses.has(x))) {
        PersonaError.softFail("Player side AE delete, may be problematic", this.toJSON());
      }
      return;
    }
    if (this.parent instanceof PersonaActor && this.parent.isValidCombatant()) {
