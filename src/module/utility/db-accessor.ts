...
export class DBAccessor<ActorType extends Actor<any, ItemType> , ItemType extends Item<any>> {
  ...
  find<const T extends BasicAccessorTypes>( accessor: UniversalAccessor<T>) : T extends infer R ? R : undefined {
  ...
  getUniversalAccessor<T extends BasicAccessorTypes>(document: T) : UniversalAccessor<T>{
    switch (true) {
  ...
  accessorEq<T extends AccessorList[keyof AccessorList]> ( a: U<T>, b: U<T>) : boolean {
    if (!a || !b) {return false;}
...
declare global {
  ...
  type UniversalAccessorTypes = Prettify<AccessorTypes[keyof AccessorTypes]>;


  type BasicAccessorTypes = BAccessorTypes[keyof BAccessorTypes];

  interface BAccessorTypes {
    "actor": Actor;
    "item" : Item;
  ...
  interface AccessorTypes extends BAccessorTypes { }
  ...
  type UniversalAccessor<T extends UniversalAccessorTypes = UniversalAccessorTypes> =
    ...
    interface AccessorList {
      ...
      "tokenDocument": UniversalTokenAccessor;
      "activeEffect" : UniversalAEAccessor;
    }
