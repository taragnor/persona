...
export class PersonaItem extends Item<typeof ITEMMODELS, PersonaActor, PersonaAE> {
  ...
  static getAllGrantedPowers (eff: ConditionalEffectC, user: ValidAttackers, situation ?: Situation) : Power[] {
    if (!situation) {
      situation = {
        user: user.accessor
      };
...
addsStatus(this: Usable, statusIds: StatusEffectId, usePowerScore ?: boolean) : number;
addsStatus(this: Usable, statusIds: StatusEffectId[], usePowerScore ?: boolean) : number;
addsStatus(this: Usable, statusIds: StatusEffectId | StatusEffectId[], usePowerScore = false) : number {
  const ids = Array.isArray(statusIds) ? statusIds : [statusIds];
  const statusesGranted = this.statusesAdded();
  return statusesGranted.reduce( (acc,st) => acc + (ids.includes(st.status)? (usePowerScore ? st.potency  : 1): 0), 0);
}
