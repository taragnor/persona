...
import {ConvertableDamageLevel, DamageSystemBase} from "./damage-system.js";
...
export class MainDamageSystem extends DamageSystemBase {
  ...
  public getWeaponSkillDamage(power: ItemSubtype<Power, 'weapon'>, userPersona: Persona, situation: SituationTypes.BonusQuerySituation) : DamageCalculation {
    ...
    const variance  = (this.BASE_VARIANCE + weaponVariance + skillDamage.extraVariance + bonusVariance );
  ...
  protected weaponVsArmorMult(attackerPersona: Persona, targetPersona: Persona) : DamageCalculation {
    ...
    const mult = Math.round(weaponValue / armorValue * 100) / 100;
    calc.add("stackMult", mult, bonusStr);
    return calc;
  }

  getWeaponRatingByWpnLevel(weaponLevel: number): number {
    return Math.round(25 * Math.pow(1.2, weaponLevel));
  }

  getArmorRatingByItemLvl(armorLvl: number) : number {
...
const DAMAGE_LEVEL_NEW = {
  ...
  "light": {extraVariance: 0, baseAmt: 15, mult: 1.2, healMult: 1.25},
  "medium": {extraVariance: 0, baseAmt: 20, mult: 1.4, healMult: 2},
  "heavy": {extraVariance: 0, baseAmt: 25, mult: 1.75, healMult: 2.75},
  "severe": {extraVariance: 0, baseAmt: 30, mult: 2, healMult: 3.5},
  "colossal": {extraVariance: 0, baseAmt: 30, mult: 2.333, healMult :4.5},
