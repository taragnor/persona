...
export class PersonaSFX {
  ...
  static onDamage(token: U<PToken>, hpchange: number, damageType :RealDamageType, _power: UsableAndCard) {
    if (!token) {return;}
    try {
      if (hpchange < 0 && token?.actor?.persona().elemResist(damageType) == "resist") {
        PersonaAnimation.queue.quickScrollingText(token.actor, 2, "RESIST", "white", 1000);
      }
