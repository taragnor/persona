...
export class PersonaCombat extends Combat<ValidAttackers, PersonaCombatant> {
  ...
  async checkForConsecutiveCombat() : Promise<boolean> {
    try {
      ...
      if (this.consecutiveCombat >= 2) {return false;}
...
private hourglass( start: boolean) {
  ...
  static solveEffectiveTargets<T extends keyof Omit<TargettingContextList, "situation">>(applyTo :T, situation: Situation, cons?: Sourced<object>) : readonly (ValidAttackers | ValidSocialTarget)[]  {
    switch (applyTo) {
      case 'target' : {
