...
class SocialCardSchema extends foundry.abstract.TypeDataModel {
  ...
  static override defineSchema() {
    const ret = {
      ...
      jobPay: new sch( {
        base: new num({initial: 0, integer: true}),
        token: new num({initial: 0, integer:true})
      }),
...
export class ConditionalEffectDM extends foundry.abstract.DataModel {
  ...
  static migrateConsequences(data: ConditionalEffect["consequences"]) {
    const arr = ConditionalEffectManager.ArrayCorrector(data);
    return arr.map(cond=> ConsequenceConverter.convertDeprecated(cond));
  }
...
export class CardChoiceDM extends foundry.abstract.DataModel {
  static override defineSchema() {
    return {
      name: new txt({initial: "New Choice"}),
      conditions: new arr(new obj<Precondition>()),
